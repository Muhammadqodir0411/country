...
async function findCountry(country) {
    ...
    if (response.status === 404) {
        $('.info').innerHTML = "<h1 class='search-title text-center w-100'> BUNDAY DAVLAT YO'Q</h1>"
    } else {

        $('.info').innerHTML = `<h1 class='search-title text-center w-100'> Qidiruv natijasi:${data.length}</h1>`;
        dataRender(data)
    }
}
